...
export default function ProcessDetailPage({
  ...
  if (!process) {
    return (
      <div>
        <p className="text-gray-500">Süreç bulunamadı.</p>
      ...
      <div className="mb-6 rounded-lg border bg-white p-4">
        ...
        <dl className="space-y-1 text-sm">
          <div className="flex gap-2">
            <dt className="text-gray-500">Süreç ID:</dt>
        ...
        <dl className="space-y-1 text-sm">
          {Object.entries(process.data).map(([fieldId, value]) => (
            <div key={fieldId} className="flex gap-2">
              <dt className="text-gray-500">{fieldLabel(fieldId)}:</dt>
              <dd>{String(value)}</dd>
            </div>
          ))}
        </dl>
          ...
          <div className="flex gap-2">
            {nextStatuses.map((status) => (
              <Button
                ...
                onClick={() =>
                  updateStatus(process.id, status, user?.username ?? "bilinmiyor")
                }
              >
